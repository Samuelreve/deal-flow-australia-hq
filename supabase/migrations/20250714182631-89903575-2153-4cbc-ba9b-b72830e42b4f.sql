...
ON public.docusign_tokens 
FOR SELECT 
USING (auth.uid() IS NOT NULL AND (auth.uid() = user_id OR user_id = 'system'::uuid));
...
-- Allow system tokens to be created by edge functions
CREATE POLICY "Allow system token creation" 
ON public.docusign_tokens 
FOR INSERT 
WITH CHECK (user_id = 'system'::uuid);
...
const NestedReplies: React.FC<NestedRepliesProps> = ({ 
  ...
  return (
    <div className="space-y-3 mt-3 relative">
      {/* Main continuous vertical line connecting all replies to parent */}
      <div className="absolute -left-6 top-0 bottom-0 w-0.5 bg-primary/60"></div>
      ...
      {replies.map((reply, index) => {
        ...
        return (
          <div key={reply.id} className="relative">
            {/* Horizontal connector from main line to comment */}
            <div className="absolute -left-6 top-4 w-6 h-0.5 bg-primary/60"></div>
            
            {/* Connection dot */}
            <div className="absolute -left-7 top-3.5 w-2 h-2 bg-primary rounded-full border border-background"></div>
            
            <div className="ml-2 transform transition-all duration-200 hover:translate-x-1">
            ...
            {/* Nested Replies - Continue the line system */}
            {reply.replies && reply.replies.length > 0 && (
              <div className="relative mt-4 ml-4">
                {/* Extend the main line for deeper nesting */}
                <div className="absolute -left-10 top-0 bottom-0 w-0.5 bg-primary/60"></div>
                <NestedReplies 

  ...
  return (
    <div className="mb-6">
      <div className="border-b">
        <nav className="-mb-px flex overflow-x-auto scrollbar-hide space-x-2 sm:space-x-6" aria-label="Tabs">
          {availableTabs.map((tab) => (
            <Link
              key={tab.id}
              to={tab.id === "documents" ? `/deals/${deal.id}/documents` : "#"}
              className={`
                relative whitespace-nowrap py-3 sm:py-4 px-2 sm:px-1 border-b-2 font-medium text-xs sm:text-sm flex-shrink-0
                ${activeTab === tab.id

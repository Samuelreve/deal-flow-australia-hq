...
const OAuthButtons = ({ mode, disabled = false, termsAccepted, privacyAccepted }: OAuthButtonsProps) => {
  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      toast.error(error.message || `Failed to ${mode === 'signin' ? 'sign in' : 'sign up'} with Google`);
    } finally {
  ...
  const handleAppleAuth = async () => {
    try {
      ...
      toast.error(error.message || `Failed to ${mode === 'signin' ? 'sign in' : 'sign up'} with Apple`);
    } finally {
  ...
  return (
    <div className="space-y-3">
      ...
      <Button
        type="button"
        variant="outline"
        className="w-full bg-black hover:bg-gray-900 text-white border-gray-300"
